...
/**
 * The green gradient balance card at the top of the wallet tab (Batch 4 brief §1 / design marker 09).
 *
 ...
 *
 * WHY `action` is a slot rather than a hardcoded "Add money" button: the design lays the button INSIDE
 * this card, bottom-right of the label/amount column (`flex items-end justify-between`) — the caller
 * still owns the click handler/disabled state, this card just reserves the design's layout for it.
 */
...
export function WalletBalanceCard({ balance, loading, action }: WalletBalanceCardProps) {
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary to-primary-dark p-6 text-white">
      ...
      <div className="relative flex items-end justify-between gap-4">
        <div>
          ...
          {loading ? (
            <div className="mt-3 h-9 w-40 animate-pulse rounded-md bg-white/20" aria-hidden />
          ) : (
            <p className="mt-3 font-display text-4xl font-semibold tracking-tight">{formatInr(balance)}</p>
          )}
          <p className="mt-2.5 text-sm text-white/85">Usable at checkout · refunds &amp; rewards</p>
        </div>
        {action}
      </div>
    </div>
