 ...
 *
 * WHY this lives in shared: the admin panel needs to SHOW the amount before an admin clicks Approve,
 * and the Cloud Function needs to WRITE it. Two implementations would drift, and a drifting money
 * figure is a support incident. `functions/src/growth/service/replacementRefund.ts` is a byte-identical
 * mirror (Cloud Functions cannot import this workspace package at runtime — the same reason
 * `functions/src/config/collections.ts` mirrors `config/collections.ts`).
 * KEEP THE TWO FILES IN LOCKSTEP.
 *
 * The server's value is authoritative; the admin's is a preview only.
 *
...
/** Round to 2 decimals so rupee amounts never carry binary-float dust. Mirrors `round2` in functions. */
...
export function computeReplacementRefund(
  order: Pick<OrderProps, 'items' | 'subtotal' | 'couponDiscount' | 'gstAmount'>,
  request: ReplacementRefundInput,
): ReplacementRefundBreakdown {
  const items: OrderItemProps[] = order.items ?? [];
  ...
  const base =
    order.subtotal > 0 ? order.subtotal : round2(items.reduce((sum, i) => sum + i.subtotal, 0));
